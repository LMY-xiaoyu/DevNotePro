              ...
              {openNoteIds.map(id => {
                ...
                return (
                  <div key={id} onClick={() => handleSelectNote(id)} onContextMenu={(e) => handleTabContextMenu(e, id)} draggable onDragStart={(e) => e.dataTransfer.setData('application/devnote-tab', id)} onDragEnd={(e) => handleTabDragEnd(e, id)} className={`group relative flex items-center gap-2 px-4 py-2.5 min-w-[120px] max-w-[200px] text-xs cursor-pointer border-r border-zinc-300 dark:border-zinc-800 select-none transition-colors ${isActive ? 'bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 font-medium border-b-0' : 'bg-zinc-100 dark:bg-zinc-950 text-zinc-700 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-900'}`}>
                    ...
                    <FileText size={12} className={isActive ? 'opacity-100' : 'opacity-50'} /><span className="truncate flex-1">{note.title || (note.content.length > 0 ? note.content.substring(0, 20) + (note.content.length > 20 ? '...' : '') : '无标题')}</span>
